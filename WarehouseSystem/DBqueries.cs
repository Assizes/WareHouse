...
namespace WarehouseSystem
{
    class DBqueries
    {
        ...
        internal string getAllBins = "SELECT bin_ID AS `ID` FROM s2016_user1.bins";
        internal string getShelfBins = "SELECT bin_ID AS `ID`, availableWidth AS `Available Width`, availableWeight AS `Available Weight`, COUNT(item_ID) AS `Number of items stored` FROM s2016_user1.bins AS BI LEFT JOIN s2016_user1.item AS IT ON BI.bin_ID = IT.FK_bin WHERE FK_shelf=@shelf GROUP BY bin_ID " +
                                        "UNION SELECT bin_ID AS `ID`, availableWidth AS `Available Width`, availableWeight AS `Available Weight`, COUNT(item_ID)AS `Number of items stored` FROM s2016_user1.bins AS BI RIGHT JOIN s2016_user1.item AS IT ON BI.bin_ID = IT.FK_bin WHERE FK_shelf=@shelf GROUP BY bin_ID";
        ...
        internal string deleteInv = "DELETE FROM s2016_user1.item WHERE item_id=@itemID";
        //testfor first one
